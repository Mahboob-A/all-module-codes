...
// see dijkstra_code. this code tells distance is ambiguous 
...
void dijkstra(int src)
{
        ...
        for(int i=0; i<n; i++)
        {
                ...
                for(int j=1; j<=n; j++)
                {
                        ...
                        if(selected_node == -1  || distance[selected_node] > distance[j])    // or distance[j] < distance[selected_node]
                        {
                                selected_node = j;
                        }
                }
                ...
                for(auto[adjNode, edge_cost] : adjList[selected_node])   // from c++17 we can extract pair value like this way 
                {
                        ...
                        if(distance[selected_node] + edge_cost < distance[adjNode])
                        {
                                distance[adjNode] = distance[selected_node] + edge_cost;
                        }
                }
        } 

    ...
    {
        if (pos == 0)    // if pos is 0, that means the new node will be new head, and prev head will be in 2nd place
        {
    ...
    void listDelete(Node* &head, int pos)  // to delete, we need address of main's head pointer and the pos to delete
    {
        ...
        pos--;    // if pos to delete is not the head node then just as before described, go to the just previous node of which we need to delete
        Node* tmp = head;
        for (int i=0; i<pos; i++)
        {
            tmp = tmp -> next;
    ...
    void printList(Node *head)  // as usual a print function that just takes the head
    {
        Node *tmp = head;

        while (tmp != NULL)   // move forward until find null
        {
            cout<<tmp -> val<<" ";

    ...
    */
    ...
    void insertAtSpecificPos(Node *&head, int pos, int val)
    {
        ...
        // corner case: this snippet allows to add node if the head is null and return if head is null and pos > 1.
        if (head == NULL && pos>1)
        {
    ...
    */
    ...
    void makeCycle(Node *&head, int pos)
    {
        ...
        if (head == NULL || head -> next == NULL)
        {
            if (head == NULL) cout<<"Head is empty"<<endl;
            else cout<<"List has not enough nodes to create a cycle"<<endl;
            return;
        }
        ...
        // now the last node will be pointing to the node == pos and run infinity as no NULL between them
        tmp -> next = startNode;
        cout<<"Cycle creation is completed!"<<endl;
    }

    // function to detect if a cycle is present in the list
    bool detectCycle(Node *&head)
    {
        // cycle detection is done by slow - fast pointer
        // step 01: make slow and fast pointers pointing to head
        Node *slow = head, *fast = head;

        // step 02: drive the fast for even case i.e fast != NULL and odd case i.e fast -> next != NULL,
        // drive slow by one and fast by twice of slow and keep a check if slow of next and fast of next meets then return true else false
        while (fast!=NULL && fast->next!=NULL)
        {
            slow = slow -> next;
            fast = fast -> next -> next;
            ...
            // cycle check
            if (slow -> next == fast -> next) // or if (slow  == fast )
            {
                return true;
            }
        }
    ...
    void removeCycle(Node *&head)
    {
        // step 01: make slow and fast pointer pointing to head
        Node *slow = head, *fast = head;

        // step 02: drive slow fast pointer loop untill slow == fast
        // as both of them begins from same position head, so while (slow != fast) will work do at first we need to alter them then begin slow fast pointer
        do
        {
            slow = slow -> next;
            fast = fast -> next -> next;
        ...
        // step 03: now as both of them are at same node, make any pointer pointing to head
        fast = head;

        // step 04: as fast is poiting to head now as slow is still at somewhere else, move them
        // by same speed i.e by one Node until slow -> next == fast -> next. So, run the loop with
        // slow -> next != fast -> next condition and move them by one Node
        ...
        // step 05: at both of the stopped before the startNode, i.e as fast was assignined to head hence
        // first is just before the startNode and slow is at the last Node
        // as slow is now at the last node, so, override last node's next to NULL as it has the address of startNode
        // of the cycle hence put slow -> next = NULL
        slow -> next = NULL;
    }
    ...
    int main() {
        Node *head = NULL;
            ...
            <<endl<<"Choice 17: Detect Cycle In A List"
            <<endl<<"Choice 18: Remove Cycle From A List"
            <<endl<<"Choice 19: Print the List"
            <<endl<<"Choice 0: Stop";
            ...
            switch(choice)
            {
                ...
                */
                ...
                case 17:
                    ...
                    if (cycleStatus) cout<<"There is a cycle in the list"<<endl;
                ...
                case 18:
                    ...
                    if (cycleStatus)
                    {
                        removeCycle(head); // calling the function to remove the cycle only if there is a cycle
                        cout<<"Cycle removal is completed!"<<endl;
                    }
                    else cout<<"There is no cycle in the list"<<endl;
                    break;
